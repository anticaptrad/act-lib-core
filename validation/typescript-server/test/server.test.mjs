...
const validActor = { userId: "user-1", tenantId: "tenant-1", roles: ["reader"] };
const validContext = { requestId: "req-1", traceId: "trace-1", actor: validActor, sourceIp: "127.0.0.1" };

test("accepts bounded trusted actor and context", () => {
...
for (const [name, actor] of [
  ["empty user id", { userId: "", roles: [] }], ["oversized user id", { userId: "u".repeat(129), roles: [] }],
  ["empty role", { userId: "user-1", roles: [""] }], ["oversized role", { userId: "user-1", roles: ["r".repeat(129)] }],
  ...
  test(`rejects trusted actor with ${name}`, () => assert.equal(TrustedActorSchema.safeParse(actor).success, false));
}
...
for (const [name, context] of [
  ...
  test(`rejects server request context with ${name}`, () => assert.equal(ServerRequestContextSchema.safeParse(context).success, false));
}

test("accepts complete internal command", () => {
  const command = { operationId: "alerts.create", idempotencyKey: "idem-1", context: validContext, payload: { message: "hello" } };
...
for (const [name, command] of [
  ...
  test(`rejects internal command with ${name}`, () => assert.equal(InternalCommandSchema.safeParse(command).success, false));
}
