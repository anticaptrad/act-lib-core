...
fn non_empty_string() -> decode.Decoder(String) {
  decode.string |> decode.then(fn(value) {
    case value == "" { True -> decode.failure("", expected: "non-empty String") False -> decode.success(value) }
  })
}

fn bounded_int(min: Int, max: Int) -> decode.Decoder(Int) {
  decode.int |> decode.then(fn(value) {
    case value >= min && value <= max { True -> decode.success(value) False -> decode.failure(0, expected: "bounded Int") }
  })
}
