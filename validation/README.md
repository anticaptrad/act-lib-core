...
This directory is the runtime validation boundary for `anticaptrad/act-lib-core`.

- Public definitions are authored in `act-interfaces` and are safe for browser, mobile, desktop, CLI, and server consumers.
- Server definitions live only here. They may extend public definitions but are never copied into `act-clients`.
