...
test("accepts request metadata at every length boundary", () => {
  assert.equal(safeParsePublic("request-meta", {
    requestId: "r".repeat(128), traceId: "t".repeat(128), locale: "l".repeat(64),
  }).success, true);
});
...
for (const [name, value] of [
  ["missing required limit", {}], ["zero limit", { limit: 0 }], ["oversized limit", { limit: 101 }],
  ["fractional limit", { limit: 1.5 }], ["empty cursor", { limit: 50, cursor: "" }],
  ["oversized cursor", { limit: 50, cursor: "c".repeat(513) }], ["unknown field", { limit: 50, offset: 1 }],
]) {
...
for (const [name, value] of [
  ["status below range", { ...validProblem, status: 399 }], ["status above range", { ...validProblem, status: 600 }],
  ["fractional status", { ...validProblem, status: 400.5 }], ["empty type", { ...validProblem, type: "" }],
  ["empty title", { ...validProblem, title: "" }], ["oversized detail", { ...validProblem, detail: "d".repeat(4097) }],
